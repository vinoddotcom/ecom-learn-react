...
describe("CheckoutPage Component", () => {
  ...
  const fillFormWithValidData = () => {
    ...
    const addressLabel = screen
      ...
      .find(
        el =>
          el.tagName.toLowerCase() === "label" && !el.textContent?.toLowerCase().includes("email")
      );
    if (addressLabel) {
  ...
  const renderCheckoutPage = (withItems = true) => {
    ...
    if (!withItems) {
      ...
      mockCartItems = [...sampleCartItems];
      mockCartTotal = sampleCartItems.reduce(
  ...
  it("handles network error during order submission", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/An error occurred while processing your order/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("calculates order total correctly", () => {
    ...
    const shipping = 5.0; // Standard shipping
  ...
  it("updates total when delivery method changes", () => {
    ...
    const subtotal = 99.99 * 2 + 49.99;
    const standardShipping = 5.0;
    const expressShipping = 16.0;
    const tax = Math.round(subtotal * 0.08 * 100) / 100;
